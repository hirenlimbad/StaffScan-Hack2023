...
def login_view(request):
    if request.method == 'POST':
        ...
        valid = employeeManagement().checkCredentials(username, password)
        if valid:
            employees = employeeManagement().showAllEmployees()
...
def add_employee(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            employeeManagement().saveEmployeeForm(form)
            employees = employeeManagement().showAllEmployees()
            isPresent = employeeManagement().isPresent()
            return render(request, 'showEmployee.html', {'employees': employees, 
                                                         'isPresent': isPresent})
...
def assign_leave(request):
    ...
    leave_requests = db.reference('leave_requests').get()
    # leave_requests will be a dictionary containing your dat
    # Pass leave_requests to the template for rendering
